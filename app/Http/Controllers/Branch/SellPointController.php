...
class SellPointController extends Controller
{
    ...
    {
        $products         = Product::query()->with(['branchWishStock', 'branchWishStockItem', 'unit', 'category', 'brand'])->where('is_deleted', 0)->paginate(10);
        $transformedItems = $products->getCollection()->map(function ($item) {
            return [
                'id'                 => $item->id,
                'name'               => $item->name,
                'price'              => 500,
